...
def main(args):
    ...
    writer = pd.ExcelWriter('classification_scores_mnist_vae.xlsx', engine='xlsxwriter')
    ...
    # define model
    encoder, decoder, model = vae(input_shape=x_train.shape[1:], latent_dim=2)
    print(model.summary())
    plot_model(model, to_file='basicVAE.png', show_shapes=True)
    plot_model(encoder, to_file='basicVAE_encoder.png', show_shapes=True)
    plot_model(decoder, to_file='basicVAE_decoder.png', show_shapes=True)
    # train model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net', type=str.lower, default='basicvae', help='Define network name.')
